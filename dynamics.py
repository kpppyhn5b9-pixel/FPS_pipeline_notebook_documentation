...
def attention_delta_fn(fn_t: np.ndarray, theta: np.ndarray, s: np.ndarray, K0: float, kappa: float,
                       garde: float, threshold: float = 0.05, bridge: Optional[np.ndarray] = None) -> np.ndarray:
    """
    ...
    îlot (un seul champ moyen, une seule moyenne de fréquence), même éloignés le long de la chaîne.
    """
...
def cherished_dream_substrate(st: Dict[str, Any], rloc: np.ndarray, dt: float, tau: float = 5.0,
                              min_size: int = 3) -> Optional[np.ndarray]:
    """
