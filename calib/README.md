...
| `run_surprise_attention.py` | mémoire de soi à deux vitesses (Aₙ/Ā, fₙ/f̄), surprise, attention : mode `gesture` (rythme vers les voisines calmes, inerte) ou `consolidate` (la mémoire longue des strates saillantes apprend plus vite) ; événements durable (f₀ × facteur) et bref. Câblé dans le pipeline le 21/09 (`memory`/`attention` de `config.json`, note *duodecies* de `docs/DOUBLONS_metriques.md`) : dans le pipeline la saillance de surprise est relative à la médiane du chœur, pas absolue comme ici | `python run_surprise_attention.py <nom> <K> <T> [seed] [tau_s] [tau_l] [facteur] [mode]` |
| `run_considerance_structures.py` | la QUESTION (21/09) : un contexte se pose, tient, migre, disparaît ; quatre lectures sans barème (engagement, sélectivité, réversibilité, coût → efficience = susceptibilité) sur sept cascades (dorée, ratio 1.5, ratio 1.0, hasard, aucune, témoins d'échelle `_x25`) ; note `docs/CONSIDERANCE_banc_structures.md` | `python run_considerance_structures.py <nom> <structure> [seed] [N] [T] [K0] [kappa] [largeur]` |
| `considerance_bench.py` | cœur partagé du banc considérance : protocole (projecteur 20 → 80 → 50 → extinction), geste, lectures substrat + engagement / sélectivité / réversibilité / coût ; lentille Rloc fixe n±1 | module, `run_bench(...)` |
| `run_ablations.py`, `table_ablations.py` | carte d'ablation des éléments de design (respiration, phases, signatures, couplage, enveloppe, latence, G, β, mémoire, générique) ; tableau | `python run_ablations.py <ablation|all> [seed] [N] [T] [K0]` ; `python table_ablations.py <dossier> [markdown]` |
| `plot_considerance_structures.py` | kymographes Rloc(t, n) + surprise des strates liées, un panneau par dossier | `python plot_considerance_structures.py <png> <dossier> ...` |
| `plot_context_binding.py`, `plot_surprise_attention.py` | figures des deux bancs précédents (`kymo.npz`, `trace.npz`) | voir en-tête |
