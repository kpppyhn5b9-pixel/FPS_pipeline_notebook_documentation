...
usage: python run_insitu_memoire.py <cheri|temoin|present|douleur_porte|douleur_sans_porte|all> [seed] [N]
"""
...
def run(name, seed, N, T, foyers, noise, attach=True, porte=True):
...
def read(d, label, wins):
    t = d['t']; N = d['O'].shape[1]; za, zb = zone(N, 20), zone(N, 80)
    print(f"[{label}] fenêtre | m 20 / m 80 | silence | rappel : 20 / 80 (% temps) | porte | zone 20 : s · R · audib | zone 80 : s · R · audib | activité/repos")
    for (a, b, tag) in wins:
        w = (t >= a) & (t < b)
        r20 = 100 * np.mean(np.isfinite(d['rappel'][w]) & (np.abs(d['rappel'][w] - 20) <= 8)); r80 = 100 * np.mean(np.isfinite(d['rappel'][w]) & (np.abs(d['rappel'][w] - 80) <= 8))
        ...
        print(f"[{label}] {a:3d}-{b:<3d} | {d['m'][w][:, za].mean():.2f} / {d['m'][w][:, zb].mean():.2f} | {100 * np.nanmean(d['silence'][w]):3.0f} % | {r20:3.0f} / {r80:3.0f} | {gv:.2f} | {zz(za)} | {zz(zb)} | ×{np.nanmedian(d['act'][w]):.2f}  {tag}")
...
if __name__ == '__main__':
    ...
    for nm in (list(MODES) if which == 'all' else [which]):
        ...
        read(d, f"{nm} s{seed}", WIN['present' if nm == 'present' else ('douleur' if nm.startswith('douleur') else 'cheri')])
    print("DONE")
